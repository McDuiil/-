...
import { Search, Play, Plus, Clock, X, Check, Award, Settings } from "lucide-react";
import GlassCard from "./GlassCard";
import { useApp } from "@/src/context/AppContext";
import { Exercise, WorkoutSession, WorkoutSessionExercise } from "@/src/types";
...
export default function Workouts() {
  ...
  return (
    <div className="space-y-6 pb-32 pt-8">
      ...
      <div className="flex items-center justify-between px-4">
        ...
        {!activeSession ? (
          <button 
            ...
            className="flex items-center gap-2 rounded-full bg-blue-500 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-blue-500/30 transition-transform active:scale-95"
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 rounded-full bg-green-500 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-green-500/30 transition-transform active:scale-95"
          >
            <Check size={16} />
            {t("endWorkout")}
          </button>
        ...
        <div className="px-4">
          <GlassCard className="space-y-6 border-blue-500/30 bg-blue-500/5">
            <div className="flex items-center justify-between">
              ...
              <input 
                type="number" 
                placeholder={t("caloriesBurned")}
                className="w-24 rounded-lg bg-white/5 px-2 py-1 text-right text-sm outline-none"
                    ...
                    <div className="space-y-2">
                      {sessionEx.sets.map((set, setIdx) => (
                        <div key={setIdx} className="flex items-center gap-3">
                          <span className="w-6 text-xs font-bold text-white/20">{setIdx + 1}</span>
                          <input 
                            type="number" 
                            placeholder={t("weightUnit")}
                            className="w-full rounded-lg bg-white/10 px-3 py-1.5 text-sm outline-none"
                            value={set.weight || ""}
                          ...
                          <button 
                            ...
                            className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${set.isPR ? "bg-yellow-500 text-black" : "bg-white/5 text-white/20"}`}
                          >
                            <Award size={16} />
                          </button>
                        </div>
            ...
            <button 
              ...
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-white/10 py-4 font-bold transition-colors hover:bg-white/20"
            >
              <Plus size={20} />
              {t("addExercise")}
            </button>
      ...
      {showExercisePicker && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-black/90 p-4 pt-12 backdrop-blur-xl">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-2xl font-bold">{t("actionLibrary")}</h2>
            ...
            {filteredExercises.map((ex, index) => (
              <GlassCard key={ex.id} className="overflow-hidden p-0" delay={index * 0.05}>
                <div className="relative aspect-[4/3]">
                  <img
                    src={appData.categoryImages[ex.part] || ex.image}

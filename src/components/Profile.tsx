...
import { Settings, LogOut, ChevronRight, Award, Target, Heart, Globe, Moon, Sun, Download, Upload, X, ChevronLeft, Flame, RefreshCw } from "lucide-react";
    ...
    <div className="space-y-6 pb-32 pt-8">
      ...
      <div className="flex flex-col items-center gap-4 px-4">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-bold">{appData.profile.nickname}</h1>
          <p className="text-sm text-white/40 dark:text-white/40 light:text-black/40">BMR: {currentBMR} kcal</p>
        </div>
      </div>
      ...
      {showEditor && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <GlassCard className="w-full max-w-sm space-y-4 border-white/20 bg-black/80 dark:border-white/20 dark:bg-black/80 light:border-black/10 light:bg-white/90">
      ...
      <div className="grid grid-cols-3 gap-3 px-4">
        <GlassCard className="flex flex-col items-center gap-1 p-3" delay={0.1}>
          <Target size={18} className="text-blue-400" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-white/40 dark:text-white/40 light:text-black/40">{t("goal")}</span>
          <span className="text-sm font-bold">{appData.profile.goalWeight}kg</span>
        </GlassCard>
        <GlassCard className="flex flex-col items-center gap-1 p-3" delay={0.2}>
          <Flame size={18} className="text-orange-400" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-white/40 dark:text-white/40 light:text-black/40">{t("dailyDeficit")}</span>
          <span className="text-sm font-bold">{Math.round(dailyDeficit)}</span>
        </GlassCard>
        <GlassCard className="flex flex-col items-center gap-1 p-3" delay={0.3}>
          <Award size={18} className="text-yellow-400" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-white/40 dark:text-white/40 light:text-black/40">{t("streak")}</span>
          <span className="text-sm font-bold">14d</span>
        </GlassCard>
      </div>
